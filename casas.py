...
def init():
    ...
    gluPerspective(60, 1, 6, 100.0)  # Campo de visión original
...
def draw_scene():
    ...
    # --- CAMBIO REALIZADO: VISTA DE AUTOMÓVIL ---
    gluLookAt(0, 1, 18,    # Eye: Bajo (Y=1) y atrás (Z=18)
              0, 1, 0,     # Center: Mirando hacia el centro a la misma altura
              0.5, 1, 0)   # Up: Inclinado (X=0.5)
...
def main():
    ...
    window = glfw.create_window(width, height, "Escena con 4 casas - Vista Auto", None, None)
    if not window:
        glfw.terminate()
        sys.exit()
